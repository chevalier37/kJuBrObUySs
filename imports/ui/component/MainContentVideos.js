...
class MainContentVideos extends Component {
...
export default withTracker(() => {
  ...
  const allposts = Videos.find({}, { sort: { date: -1 }});
  const amour = Videos.find({$or:
    [{categorie:'premierAmour'},
  	{categorie:'trahison'},
    {categorie:'Friendzone'},
    {categorie:'amourdistance'},
    {categorie:'separation'}]},
    { sort: { date: -1 }, limit:30 });

  const autre = Videos.find({categorie:'autre'},
    { sort: { post_date: -1 }, limit:30 });


  const confiance = Videos.find({$or:
  	[{categorie:'timidite'},
  	{categorie:'depression'},
    {categorie:'suicide'},
    {categorie:'deces'},
    {categorie:'mutilation'}]},
    { sort: { date: -1 }, limit:30 });

  const sexo = Videos.find({$or:
  	[{categorie:'sexo'},
  	{categorie:'Contraception'},
    {categorie:'mst'},
    {categorie:'viol'},
    {categorie:'avortement'},
    {categorie:'orientationSex'}]},
    { sort: { date: -1 }, limit:30 });

  const sante = Videos.find({$or:
  	[{categorie:'Anorexie'},
  	{categorie:'obesite'},
    {categorie:'drogue'},
    {categorie:'alcool'},
  ...
  const ecole = Videos.find({$or:
  	[{categorie:'echecEcole'},
